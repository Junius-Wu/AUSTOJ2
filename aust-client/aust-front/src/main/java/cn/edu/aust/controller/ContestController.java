    ...
    try {
      ...
      if (StringUtils.isEmpty(curContest)) {
        ...
        if (Arrays.binarySearch(ids, String.valueOf(id)) >= 0) {
          return new Result<PosCode>(PosCode.OK);
        }
      }
      //检查是否可以访问
      ...
      log.error("访问比赛出错",e);
      return new Result<PosCode>(PosCode.NO_PRIVILEGE.getStatus(),e.getMessage());
    }
    return new Result<PosCode>(PosCode.NO_PRIVILEGE);
  }
